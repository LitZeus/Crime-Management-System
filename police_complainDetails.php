...
<head>
    ...
    if(isset($_POST['status'])){
        ...
        {
            $upd=$_POST['update'];
            $qu1=mysqli_query("insert into update_case(c_id,case_update) values('$cid','$upd')",$conn);
        }
    }
    ...
    if(isset($_POST['close'])){
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $qu2=mysqli_query("insert into update_case(c_id,case_update) values('$cid','$up')",$conn);
            $q2=mysqli_query("update complaint set pol_status='ChargeSheet Filed' where c_id='$cid'",$conn);
